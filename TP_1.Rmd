...
```{r,cache=TRUE}
head(select(recorridos,dia_de_semana_origen,hora_del_dia_origen,mes_de_origen))
```
...
usuarios_diarios_anio$total <- as.numeric(usuarios_diarios_a?o$total)
usuarios_diarios_anio$dia_semana <- as.factor(strftime(usuarios_diarios_a?o$fecha_origen_auxiliar, "%A"))
...
usuarios_diarios_anio$es_feriado <- factor(ifelse(usuarios_diarios_a?o$fecha_origen_auxiliar %in% feriados,
                                                 1, 0))

tab <- usuarios_diarios_anio %>% filter(es_feriado == 0) %>% group_by(dia_semana) %>% summarise(mu = mean(total)) %>% ungroup()
tab$z <- usuarios_diarios_a?o %>% filter(es_feriado == 0) %>% group_by(dia_semana) %>% summarise(mu = qt(0.975, length(total) - 1)) %>% ungroup()
...
zcrit = qt(0.05, recorridos = n-1)

zstat;zcrit

pvalor <- pt(zstat,recorridos=n-1,lower.tail=TRUE);pvalor
```
...
xcrit <- qt(0.05, recorridos = n-1)*sqrt((0.2*(1-0.2))/n)+0.2

zcrit <- (xcrit - pstar)/sqrt((pstar*(1 - pstar))/n)

beta <- 1 - pt(zcrit, recorridos = n-1)
...
df_with_users <- select(recorridos_with_users, -c(fecha_alta,hora_alta))
